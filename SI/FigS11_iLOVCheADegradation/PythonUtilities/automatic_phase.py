...
def automatic_phase(vector=None, pivot1=1, funcmodel='minfunc', *args, **kwargs):
    ...
    if npts > 10:
        ...
        raise ValueError(
            "The input vector is a too small column vector to adjust is phase")
    phase_corrected_vector = vector*np.exp(1j*zero_phase)
    vector_imag = np.imag(vector)
    # Calculate zero and first order phase correction
    if funcmodel == 'minfunc':
        minimum = scipy.optimize.fmin(minfunc, [zero_phase, 0.0], args=(
            phase_corrected_vector, pivot1), maxiter=1000, maxfun=1000)
    elif funcmodel == 'acme':
        minimum = scipy.optimize.fmin(acme, [zero_phase, 0.0], args=(
            phase_corrected_vector, pivot1), maxiter=1000, maxfun=1000)
    else:
        warn("There is only two options for the funcmodel: 'minfunc' and 'acme'."
             "By default, 'minfunc' was used.")
        minimum = scipy.optimize.fmin(
            minfunc, [zero_phase, 0.0], args=(phase_corrected_vector, pivot1))
    phi0, phi1 = minimum
    q = 1j*np.pi/180
    First = -1*(np.arange(0, npts)-pivot1)/(npts)
    phase_parameters = {'zero_phase': zero_phase*180/np.pi,
                        'first_phase': (phi0*180/np.pi, phi1*180/np.pi)}
    phase_corrected_vector = np.multiply(
        phase_corrected_vector, np.exp(q*(phi0+First*phi1)))
...
def minfunc(phase, complex_vector, pivot1):
    ...
    complex_vector_corr = np.multiply(
        complex_vector, np.exp(q*(phi0+First*phi1)))
